# -*- coding: utf-8 -*-
"""
Created on Wed Sep 18 14:00:24 2019
...
plt.tight_layout()
plt.savefig('C-NN-INV-FIG/Exact-Burgers.png',dpi=1000)
#plt.show()
plt.close(fig)
...
x_exa0 = np.linspace(-8.0,8.0,Nu).reshape((-1,1))
...
xu_train = np.concatenate((xu_train, np.concatenate((x_exa0,1.5*np.ones((x_exa0.shape[0],1))),axis=1)),axis=0)
xu_train = np.concatenate((xu_train, np.concatenate((x_exa0,1.6*np.ones((x_exa0.shape[0],1))),axis=1)),axis=0)
...
NN_instance.training(num_iter=50001)
...
plt.xlabel('t')
plt.ylabel('x')
plt.title('1D Burgers\' equation: Continuous time NN-inverse (solution)')
plt.tight_layout()
plt.savefig('C-NN-INV-FIG/C-NN-Burgers-solution-'+str(Nf)+'.png',dpi=1000)
plt.close(fig)
...
plt.xlabel('t')
plt.ylabel('x')
plt.title('1D Burgers\' equation: Continuous time NN-inverse (absolute error)')
plt.tight_layout()
plt.savefig('C-NN-INV-FIG/C-NN-Burgers-Ab-err-'+str(Nf)+'.png',dpi=1000)
plt.close(fig)


np.savetxt('C-NN-INV-FIG/exact_u.txt', u_exa, fmt='%10.5e')    
np.savetxt('C-NN-INV-FIG/predicted_u.txt', u_pred, fmt='%10.5e')    

u_error = np.linalg.norm(u_exa[:,:51].reshape((-1,1))-u_pred.reshape((-1,1)))/np.linalg.norm(u_exa[:,:51].reshape((-1,1)))
print('u_error= ', u_error)
np.savetxt('C-NN-INV-FIG/u_error.txt', [u_error], fmt='%10.5e' )
